...
console.log("Module baseurl: ", baseurl);
...
//module.exports = {
export async function getDb(nexmo, dbname = 'nexmo') { // Uses local .env file for credentials.
...
export function nidsLog(nexmo, demo = '', event = '', status = '', results = '', comment = '') {
    /*
...
export function nexmoLogout(db, id) {

}
export async function getNexmo(userId) {
...
export async function getNexmoFromNumber(nexmo, pto) {
    const attrs = {};
    return attrs;
}

export function getIdFromJWT(nexmo, jwt = '', req = {}) {
    if (!jwt.length) {
...
export function getFromJWT(nexmo, jwt = '', param = 'userid') {
    if (!jwt.length) {
        console.log("jwt not passed into function, retrieve it.");
        jwt = this.getBearerToken(req);
    }
...
export async function async(number, url, type = 'incoming') {
...
export default function vutils() { }
//}
